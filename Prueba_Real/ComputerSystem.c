...
void Computer_PrintProgramList();
void ComputerSystem_ShowTimeCS(char);
int Clock_GetTime();
...
void ComputerSystem_PowerOff() {
	...
	ComputerSystem_ShowTimeCS(HARDWARE);
...
void Computer_PrintProgramList(){
	...
	int i;
	for (i=0; i<USERPROGRAMSMAXNUMBER && userProgramsList[i]!=NULL; i++){
		ComputerSystem_DebugMessage(102,INIT,userProgramsList[i]->executableName,userProgramsList[i]->arrivalTime);
	}
}

// Show time messages for Computer System
void ComputerSystem_ShowTimeCS(char section) {
