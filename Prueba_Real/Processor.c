...
void Processor_ShowTimeProcessor(char);
...
// Processor registers: getters and setters
// int Processor_GetAccumulator();


int registerA_CPU; // General purpose register	// Register that contains the type of the system call
int registerB_CPU; // V4, ex 1a // Register that contains the type of the exception
...
void Processor_FetchInstruction() {
	...
	Processor_ShowTimeProcessor(HARDWARE);
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (registerIR_CPU.operationCode) {
		...
		// Instruction HALT
		case 'h':
			if(!Processor_PSW_BitState(EXECUTION_MODE_BIT)) Processor_RaiseException(INVALIDPROCESSORMODE);
			else Processor_ActivatePSW_Bit(POWEROFF_BIT);
			break;
...
// Show time messages for processor
void Processor_ShowTimeProcessor(char section) {
