...
void OperatingSystem_MoveMemoryFromTo(int from, int to, int size) {

int i;
MEMORYCELL emptymemoryCell;
emptymemoryCell.operationCode=emptymemoryCell.operand1=emptymemoryCell.operand2=0;

	for (i=0; i<size; i++){
		Processor_SetMAR(from);
		Buses_write_AddressBus_From_To(CPU, MAINMEMORY);
		...
		Processor_SetMAR(from++);
        Buses_write_AddressBus_From_To(CPU, MAINMEMORY);
        Processor_SetMBR(&emptymemoryCell);
        Buses_write_DataBus_From_To(CPU, MAINMEMORY);
		MainMemory_writeMemory();
	}
}